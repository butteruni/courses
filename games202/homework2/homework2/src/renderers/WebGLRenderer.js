        ...
        for (let l = 0; l < this.lights.length; l++) {
            ...
            for (let i = 0; i < this.meshes.length; i++) {
                ...
                for (let k in this.meshes[i].material.uniforms) {
                    ...
                    let precompute_L_rotation_RGB = getRotationPrecomputeL(precomputeL[guiParams.envmapId], cameraModelMatrix);
                    
                    Mat3Value =  getMat3ValueFromRGB(precompute_L_rotation_RGB);
                    for(let j = 0; j < 3; j++){
                        if (k == 'uPrecomputeL['+j+']') { 
                            gl.uniformMatrix3fv(
                                this.meshes[i].shader.program.uniforms[k],
