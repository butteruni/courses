...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i in valid_words:
        if (user_word == i):
            return i
        if (mn_diff > diff_function(user_word, i, limit)):
            correct_word = i
            mn_diff = diff_function(user_word, i, limit)
    if (mn_diff > limit):
        correct_word = user_word
    return correct_word
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    time = [[] for i in range(len(times_per_player))]
...
def fastest_words(game):
    ...
    for i in word_indices:
        mn_time, mn_id = 1e7, 1e7
        for j in player_indices:
            if mn_time > all_times(game)[j][i]:
                mn_time = all_times(game)[j][i]
                mn_id = j
        result[mn_id].append(all_words(game)[i])
    return result
